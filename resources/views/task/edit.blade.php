                    ...
                    <form action="{{ route('task_edit', ['task_id' => $task->id]) }}" method="post">
                        ...
                        <div class="mb-3">
                            <div class="form-check form-check-inline">
                                <input type="radio" id="finish_flg1" name="finish_flg" class="form-check-input" value="0" {{ $task->finish_flg == 0 ? 'checked' : '' }}>
                                <label for="finish_flg1" class="form-check-label">未完了</label>
                            </div>
                            <div class="form-check form-check-inline">
                                <input type="radio" id="finish_flg2" name="finish_flg" class="form-check-input" value="1" {{ $task->finish_flg == 1 ? 'checked' : '' }}>
                                <label for="finish_flg2" class="form-check-label">完了</label>
                            </div>
                        </div>
                        <div class="mb-3">
                            <label for="fionish_date" class="form-label">完了日</label>
                            <input type="date" id="finish_date" name="finish_date" class="form-control" value="{{ optional($task->finish_date)->format('Y-m-d') }}">
                        </div>
